...
| `docs/specs.md` | 現状のシステム仕様。システム全体の構成と実装済みの機能を記載をすること。実装フェーズで更新してください |
| `docs/tasks.md` | 全体の作業計画．手作業で変更する． |
| `docs/personal_tasks/tasks_*.md` | 現在の作業計画。計画フェーズで作成・更新 |
...
- ファイルに記載するタスクの内容の文字列を考え，`docs/personal_tasks/tasks_YYYY_MM_DD_タスク内容.md`のような形式のファイルを作成し，計画を記載すること
...
- mainブランチにいることが確認出来たら，`git switch`を用いて新規実装のための新しいブランチを作成すること
  - ブランチ名の例：feat/機能名）
- `docs/personal_tasks/tasks_YYYY_MM_DD_タスク内容.md` に記載された内容に基づいて実装を行うこと
...
- `docs/specs.md` と `docs/personal_tasks/tasks_YYYY_MM_DD_タスク内容.md` を読み込み，実装後のコードベースについての説明が各mdファイルに網羅的に記述されていることをまず確認すること
